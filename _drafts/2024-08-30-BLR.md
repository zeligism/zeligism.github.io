...
## Bregman divergence
Looking at the second formulation of the sub-problem (2), we can write it as the minimizer of some model of the objective $\argmin_{x_t}\hat{f}(x_{t+1})$ such that
$\begin{equation}
...
![Bregman divergences and their associated functional averages](../assets/bregman_nock_nielsen_2005.png)
...
The *functional average* in the fourth column could be seen as the minimizer of the "smallest enclosing" Bregman divergence with respect to the first argument. Ignoring the "smallest enclosing" constraint and setting $\alpha_i=1/m$, the $c$ in the last column is simply $\argmin_{c} \frac{1}{m} \sum_{i=1}^m D_F(c,s_i)$. The $\alpha_i$ are just Lagrange multipliers that come from the constrained problem: $\argmin_{r,c} \ r$ such that $D_F(c,s_i) \leq r$ for $1 \leq i \leq m$ (in the spirit of support vector machines). Namely, since $\frac{\partial}{\partial c} D_{F}(c,s_i) = \nabla F(c) - \nabla F(s_i)$, the functional average is $c^* = (\nabla F)^{-1}(\frac{1}{m} \sum_{i=1}^m \nabla F(s_i))$. Indeed, this is the arithmetic average when $F$ is the L2 norm as shown in the table There is also an interesting parallel in Bayes decision theory. When the risk of a classifier $\hat{y}$ is $R(\hat{y}) = \frac{1}{2} \mathbb{E}_{x,y} \|\hat{y}(x) - y\|^2$, then the Bayes optimal classifier is $\hat{y}(x) = \mathbb{E}_{y|x}[y]$, which is like the arithmetic average. For L1 distance risk, we get the median instead of the average, but this is not necessarily the same for Bregman divergences! Remember that Bregman divergences are associated with *differentiable* and strictly convex functions (but see the last row in the table and take $p \to 1$).
...
In fact, $(\nabla \psi)^{-1} = \nabla \psi^{*}$, where $\psi^{*}$ is the convex conjugate of $\psi$. This is not clear at all, so let us see why it holds.

Recall the definition of the convex conjugate $\psi^* : \mathcal{X}^* \to \mathbb{R}$, where $\mathcal{X}^*$ is the dual space of $\mathcal{X}$
$\begin{equation}
\psi^{*}(g) \coloneqq \sup_{x} \langle g, x \rangle - \psi(x).
\end{equation}$
Let us take the supremem for each $\psi^*(g)$
$\begin{equation}
\frac{\partial}{\partial x}\left(\langle g, x \rangle - \psi(x)\right) = 0
\quad\implies\quad
g = \nabla \psi(x^{\text{opt}}).
\end{equation}$
$x^{\text{opt}}$ is a maximizer since $-\psi(x)$ is strictly concave. Thus, for $x^{\text{opt}}$, the definition holds with equality $\psi^{*}(g) = \langle g, x^{\text{opt}} \rangle - \psi(x^{\text{opt}})$.
By taking the gradient with respect to $g$, we get $\nabla \psi^{*}(g) = x^{\text{opt}}$. Thus,
$\begin{equation}
g = \nabla\psi(\nabla\psi^{*}(g))
\quad \text{and} \quad
x^{\text{opt}} = \nabla\psi^*(\nabla\psi(x^{\text{opt}})),
...
\end{equation}$
Let $x \in \mathcal{X}$ and $g \in \mathcal{X}^*$. Recall Fenchel-Young inequality (which directly follows from the definition of the convex conjugate)
$\begin{equation}
\langle x, g \rangle \leq \psi(x) + \psi^*(g),
\end{equation}$
which holds with equality when $g \in \partial\psi(x)$ (you can prove it as an exercise). Using the biconjugate $\psi^{**}$, we also have
$\begin{equation}
\langle x, g \rangle \leq \psi^{**}(x) + \psi^*(g),
\end{equation}$
which similarly holds with equality when $x \in \partial\psi^*(g)$. Thus, since $\psi = \psi^{**}$ for lower semi-continuous $\psi$, we get
$\begin{equation}
g \in \partial\psi(x) \iff x \in \partial\psi^*(g).
\end{equation}$
This is an even more general form of $(\nabla\psi)^{-1} = \nabla\psi^{*}$. For a rigorous treatment of the above, see Theorem 23.5 in [Convex Analysis (Rockafellar, 1970)](https://convexoptimization.com/TOOLS/AnalyRock.pdf).
...
$\begin{align}
&\argmin_{x}\ \max_{\|\varepsilon\|_2 \leq \rho}\ f(x + \varepsilon) + \frac{\lambda}{2}\|x\|_2^2 \tag{SAM} \\
&\argmin_{x}\ \mathbb{E}_{\varepsilon \sim \mathcal{N}(0, \sigma^2 I_d)}\ [f(x+\varepsilon)] + \frac{\lambda}{2}\|x\|_2^2 \tag{Bayes}.
\end{align}$
...
In fact, it has been shown recently that $\text{(Bayes)}$ is very much related to $\text{(SAM)}$. The answer is in the title of the paper that explains it: ["SAM as an Optimal Relaxation of Bayes"](https://arxiv.org/abs/2210.01620). Namely, $\text{(SAM)}$ is an optimal relaxation of $\text{(Bayes)}$, where the relaxation is based on the convex biconjugate $f^{**}$ and the word "optimal" comes from the fact that $f^{**}$ is the optimal convex lower bound of $f$.
...
$\begin{equation}
\argmin_q\ \mathbb{E}_{x \sim q} [f(x)] + D_{\text{KL}}(q(x)\, \|\, p(x) ), \tag{OIPR}
\end{equation}$
where p is the prior of $x$, $q$ is the posterior given data $\xi$, and $D_{\text{KL}}$ is the KL-divergence (acting as a regularizer of $q$ with respect to $p$). Assuming a faithful **negative log-likelihood (NLL) loss** $f(x) \propto -\log p(\xi|x)$, the minimizer of  $\text{(OIPR)}$ recovers the true posterior $p(x|\xi)$, which corresponds to [Bayesian inference](https://en.wikipedia.org/wiki/Variational_Bayesian_methods) as a special case (see below).

Perhaps practitioners of Bayesian methods might be slightly confused by the notation and KL-divergence, so let us clarify things a bit. First, we are sticking with the optimization notation where $f$ is the loss, $x$ is the parameter (acting as the "latent" variable), and $\xi$ is the dataset (usually written in the literature as pairs of $x_i$ and $y_i$). Another source of confusion might be $D_{\text{KL}}(q(x)\, \|\, p(x) )$ since $q(x)$ should actually approach $p(x|\xi)$ as it gets better. The corresponding [evidence lower bound (ELBO)](https://en.wikipedia.org/wiki/Evidence_lower_bound) should look something like this
$\begin{align}
\mathcal{L}(q)
&\coloneqq \mathbb{E}_{x \sim q}\left[ \log \frac{p(x,\xi)}{q(x)} \right] \\
...
$\begin{align}
\text{(OIPR)'s obj.}
&= \mathbb{E}_{x \sim q} \left[\log \frac{q(x)}{p(\xi|x)p(x)}\right] + \text{const}
\\&= \mathbb{E}_{x \sim q} \left[\log \frac{q(x)}{p(x|\xi)p(\xi)}\right]+ \text{const}
\\&= -\text{(ELBO)} + \text{const},
\end{align}$
so minimizing $(\text{OIPR})$ given $f(x) \propto -\log p(\xi|x)$ implies maximizing the ELBO, as shown in [(Khan & Rue, 2023, p. 28)](https://arxiv.org/abs/2107.04562). (There is actually a small typo there where they flipped the sign of the evidence.)

### Beyond "point" gradients

We now come to a question that represents the crux of this blog post. **Can we solve $\text{(OIPR)}$ directly with gradient descent**? What does it mean to take a gradient descent step on $q$? Does $q_{t+1} = q_t - \eta_t \nabla_q \left(\mathbb{E}_{x \sim q} [f(x)] + D_{\text{KL}}(q(x)\, \|\, p(x) ) \right)$ make sense? Yes, kind of! Not only does it make sense, we should sometimes think about gradient descent from this perspective. Let us see why next.
...
An interesting thing happens when you minimize the second-order taylor expansion of the functional $F(q) \coloneqq \mathbb{E}_{x \sim q} [f(x)]$ around the mean $\mu \coloneqq \mathbb{E}_{x \sim q} [x]$ (we will ignore the KL divergence term for now)
$\begin{align}
...
\end{align}$
Let $\Sigma \coloneqq \mathbb{E}_{x \sim q} [(x-\mu)(x-\mu)^T]$ be the covariance of $x$. Then,
$\begin{align}
...
\end{align}$
In other words, approximating the objective $\argmin_q F(q)$ given the mean and covariance gives us
$\begin{equation}
\argmin_{q\ | \ \mu,\ \Sigma} \mathbb{E}_{x \sim q} [f(x)] \approx \argmin_{\mu, \ \Sigma} f(\mu) + \| \nabla^2 f(\mu) \|^2_{\Sigma^{1/2}}.
\end{equation}$
...
Since $A(\theta)$ is strongly convex and differentiable, we have a maximum $\theta^* = (\nabla A)^{-1}(T(x))$.
But on the other hand, $\sup_{\theta} \langle \theta, T(x) \rangle - A(\theta)$ is precisely the definition of $A^*(T(x))$.
Oh wait, we have seen something like this before! Looking back at the mirror descent section, we can see that $\theta^* = (\nabla A)^{-1}(T(x)) = \nabla A^*(T(x))$. This is one of the perks of the exponential family. More on this later.


### The BLR update
The second component of the Bayesian Learning Rule is the update
$\begin{equation}
    \theta_{t+1} = \theta_{t} - \eta_t \tilde{\nabla}_{ {\color{orange}\theta_t}} f_{\text{OIPR}}(q_{ {\color{orange}\theta_t}}), \tag{OIPR-step}
\end{equation}$
where $f_{\text{OIPR}}(q) = \mathbb{E}_{x \sim q} [f(x)] + D_{\text{KL}}(q(x)\, \|\, p(x) )$ and $\tilde{\nabla}$ is the [natural gradient](https://yorkerlin.github.io/posts/2021/10/Geomopt02/).

By assuming a regularizer $\Gamma(x) = - \log p(x)$ and
noting that $D_{\text{KL}}(q(x)\, \|\, p(x) ) = \mathbb{E}_{x \sim q} [\Gamma(x)] - H(q)$, where $H(q) = -\mathbb{E}_{x \sim q}[\log q(x)]$ is the entropy,
the BLR paper writes $f_{\text{OIPR}}(q)$ in terms of the loss $f$, the regularizer $\Gamma$, and the entropy $H$ as
$\begin{equation}
    \theta_{t+1} = \theta_{t} - \eta_t \tilde{\nabla}_{ {\color{orange}\theta_t}} (\mathbb{E}_{x \sim q_{ {\color{orange}\theta_t}} } [f(x) + \Gamma(x)] - H(q_{ {\color{orange}\theta_t}})). \tag{BLR-step}
\end{equation}$

Recall that for a NLL loss, i.e., $f(x) \propto - \log p(\xi|x)$, optimizing $\text{(OIPR)}$ becomes equivalent to maximizing the ELBO, and the optimal solution is $q^* = p(x|\xi)$.
...
$\begin{equation}
    \tilde{\nabla}_{\theta} \mathbb{E}_{x \sim q_{\theta}}(\cdot)
    = F(\theta)^{-1} \nabla_{\theta} \mathbb{E}_{x \sim q_{\theta}} (\cdot).
\end{equation}$
...
$\begin{equation}
    \nabla A(\theta) = \mathbb{E}_{x \sim q_{\theta}}[T(x)]
    := \nu(\theta)
    \tag{Expect. param.}
    ,
\end{equation}$
...
$\begin{equation}
    \tilde{\nabla}_{\theta} \mathbb{E}_{x \sim q_{\theta}}(\cdot)
    = \nabla_{\nu} \mathbb{E}_{x \sim q_{\theta}} (\cdot)
    .
\end{equation}$
...
\end{equation}$
which immediately implies the previous identity since $F(\theta) = \nabla^2 A(\theta^*)$. This suggests that the natural gradient is inherently in every solution of the BLR problem.
...
$\begin{align}
    ...
    ,
\end{align}$
where the second equality follows because $\mathbb{E}[(x-\mu)(x-\mu)^T] = \mathbb{E}[xx^T] - \mu\mu^T$. Note again that $\nu$ depends on $T(x)$ and not on $\theta$.

Now we have the necessary tools to apply BLR. First, recall the BLR step
$\begin{equation}
    \theta_{t+1} = \theta_{t} - \eta_t \tilde{\nabla}_{ {\theta_t}} (\mathbb{E}_{x \sim q_{ {\theta_t}} } [f(x) + \Gamma(x)] - H(q_{ {\theta_t}})). \nonumber
\end{equation}$
Let $\Gamma(\cdot)=0$ for simplicity. We only have to figure out what $\tilde{\nabla}_{ {\theta_t}} \mathbb{E}_{x \sim q_{ {\theta_t}} } [f(x)]$ is (we already know what the gradient of entropy is given constant base measures as in Gaussians, e.g., see Appendix.)
...
With that in mind, we approximate $\mathbb{E}_{x \sim q_p} [f(x)] \approx f(\tilde{x})$. Now given the concrete reparameterization, we have $\nu = \tanh(\theta)$ and using the chain rule, we get
