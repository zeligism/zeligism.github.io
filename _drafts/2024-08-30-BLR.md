...
$$\begin{equation}
\psi^{*}(g) := \sup_{x} \langle g, x \rangle - \psi(x).
\end{equation}$$
...
&\arg\min_{x}\ \mathbb{E}_{\varepsilon \sim \mathcal{N}(0, \sigma^2 I_d)}\ [f(x+\varepsilon)] + \frac{\lambda}{2}\|x\|_2^2 \tag{Bayes}.
\end{align}$$
...
$$\begin{equation}
\arg\min_q\ \mathbb{E}_{x \sim q} [f(x)] + D_{\text{KL}}(q(x)\, \|\, p(x) ), \tag{OIPR}
\end{equation}$$
...
$$\begin{align}
\mathcal{L}(q)
&:= \mathbb{E}_{x \sim q}\left[ \log \frac{p(x,\xi)}{q(x)} \right] \\
...
Recall that for a NLL loss, i.e., $$f(x) \propto - \log p(\xi \mid x)$$, optimizing $$\text{(OIPR)}$$ becomes equivalent to maximizing the ELBO, and the optimal solution is $$q^* = p(x \mid \xi)$$.
