...
Here, instead and computing $$y$$ given $$f$$ and $$x$$,
...
If $$y$$ is bigger than 0, then we predict 1, otherwise we predict -1.
Pretty straightforward.

Let us consider a simple example.
Let $x$ be the number of hours a student studies, and $y$ is wether the grade is pass or fail (pass = +1, fail = -1).
...
It's often done with math becuase visualizing the data can only get you so far.
...
We assume the number of study hours to have the same effect.
...
This is called the **iid assumption** (independent and identically distributed), and it is a crude assumption that statisticians and machine learning researchers love to us because it simplifies things a lot.
...
Students who passed seem to be *randomly* scattered around the average passing grade.
This is clear from the histogram.

When we say *random*, we usually think of the *normal* kind of randomness, the bell-shaped curve (or the box-shaped, uniform randomness for a bounded range).
...
**For random passing student, we assume that the study hours is normally distributed around the average passing grade (and equivalently for failing students students).**

We can also use fancier distrbutions, but let's stick to normal distribution with fixed widths (i.e., variance).
...
Some modern research suggest that there is no real trade-off. Oh well.
...
You could have the equivalent of a million thresholds for this silly prediction problem of pass/fail, yet the deep learning model—a neural network—will still find the exact correct solution somehow and not overfit at all.
To make things more confusing, the same model will gladly fit complete garbage random data perfectly, so it can overfit if it wants to!

How is this possible?
Would you be surprised to tell you that **nobody knows**?
It's sadly true, but it's also exciting.
This is one of the most interesting question in deep learning, and we still don't a satisfying explanation.
...
One of the rare exceptions is linear regression, where the objective can be optimized directly and a closed form solution is attainable.
...
we interpret it as a data generating process $$\Pr[\textsf{pass}] \to \textsf{grade}$$.
