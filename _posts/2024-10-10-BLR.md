...
However, $$A^T A$$ can be very large and expensive to invert, $$x$$ can be stochastic and streaming, or we are facing numerical instabilities using our closed-form solution.
Thus, we might have to resort to *approximations* using iterative methods ($$x_0 \to x_1 \cdots \to x_t$$).
...
The king of approximation when $$f$$ is differentiable is the [Taylor expansion](https://en.wikipedia.org/wiki/Taylor_series), which is found in every scientist's toolbox.
...
A gradient descent **step** with *step size* $\eta_t$ at a point $$x_t$$ simply minimizes the *second-order* approximation of $$f$$ around $$x_t$$, assuming a sharper (well-shaped) curvature as the step size becomes smaller, i.e., $$\nabla^2 f(\bx_t) \approx \frac{1}{\eta_t}I$$
...
The assumption on the Hessian (the techinical term for curvature) $$\nabla^2 f(\bx_t) \approx \frac{1}{\eta_t}I$$ looks weird but should become clearer as we proceed.
The above is a sub-problem of the original problem.
It decomposes the main problem into a series of smaller sub-problems around small neighborhoods, refining the solution step by step.
Solving it explicitly by finding $$x_{t+1}$$ at which the gradient of the sub-problem's objective is 0, we get gradient descent step
...
Indeed, *the curvature characterizes the metric*, which influences how we take optimal step.
...
Let's now walk through specialized versions of gradient descent that work better on structured space.
...
So, is there a specific rule for choosing the distance (i.e., the second order term in the gradient descent sub-problem)?
Kind of.
...
The Bregman divergence is quite simple the *second and higher-order* terms of a strictly convex and differentiable function $$\psi$$.
