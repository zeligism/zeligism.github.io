...
BLR is an elegant connection between optimization and Bayesian inference, which is slightly reminiscent of the one between ridge regression and the maximum a posteriori estimator given a Gaussian prior, but it's much more general.

There is some math involved, and the reader **cannot** get away from skipping it. Sorry, but it's the harsh truth.
Maybe you can take a look at Figure 1.
It won't really give you the accurate intuition you need, but hey, at least it doesn't have math in it.
...
This will serve as a motivation and will give some intuition for the next section, which is about an information-theoretic objective that formulates the optimization problem with respect to distributions instead of hard iterates.
Next, we will introduce the Bayesian Learning Rule (BLR) by [Khan & Rue (2023)](https://arxiv.org/abs/2107.04562).
This is a recent paper that compiles many results from previous works by Khan and colleagues under one framework.
The latter sections show derivations for different algorithms based on the BLR and conclude with some discussion.

Overall, the BLR is a fresh perspective on optimization (and inference) and can shed light on some practices and help design better and more specialized algorithms. Let's go!

[*Note: This is a long post.
You can skim through it to get a sense.
I will consider breaking it into 2 or 3 posts, but will keep it like this for the time being.*]
...
# Descending a valley
Suppose we are given the following: (1) a point $$\bx_t \in \mathcal{X}$$ for iteration $$t$$ (your current location in the valley),
(2) a loss function $$f : \mathcal{X}\to\mathbb{R}$$ (the valleys's topology),
and (3) a step size $$\eta_t$$ (or rather, the "strength" of your step).
*Gradient descent* is a principled way to descend the valley $$f$$ one *step* at a time.
The *step* can be formulated as the solution of following sub-problem:
find the lowest point of a second-order approximation of $$f$$ around $$\bx_t$$ assuming that $$\nabla^2 f(\bx_t) \approx \frac{1}{\eta_t}I$$.
...
This gives the gradient descent step as the solution
...
In layman's terms, you check the steepest direction with your stick, calculate its slope magnitude, and then jump $$\eta_t \|\nabla f(\bx_t)\|$$ units in that direction (don't think of it as "jumping down", think of it as moving yourself a fixed distance on a contour map).
Due to the $$\|\nabla f(\bx_t)\|$$, we have more freedom to jump forward when the slope is steep, but we are careful with our steps as we might overshoot the minimum when the slope is almost flat. Makes sense so far.
...
*Figure 1: Descending an ill-shaped, non-convex valley, but you're blindfolded (and you have a stick).*

A critical observation about the above is that *the curvature informs the metric under which we take the optimal step*.
The metric in the sub-problem above is Euclidean because the Hessian is $$I$$ (scaled by the inverse step size).
If we assume that the Hessian is $$D_t$$, then we get the [Mahalanobis distance](https://en.wikipedia.org/wiki/Mahalanobis_distance) instead $$\|\bx-\bx_t\|_{D_t} = \sqrt{\langle \bx-\bx_t, D_t(\bx-\bx_t) \rangle}$$.
...
Consider the different scales of the *layers and components* in deep neural nets as opposed to the *whole neural net*, for example.
...
If we have some *prior knowledge* about $$\mathcal{X}$$, then we might be able to come up with better metrics.
...
Using $$D_t = \frac{1}{\eta_t} \nabla f(\bx_t) \nabla^\top f(\bx_t)$$ recovers AdaGrad. This choice minimizes (in hindsight) an optimization quanitity called *regret*, which is a surrogate of $$f(\bx_t) - f(\bx^*)$$ for convex $$f$$.

In this post, I will talk about another interesting approach that leads to similar algorithms but based on very different motivations and assumptions. Before that, it is better to walk through specialized versions of gradient descent that work better on structured space.
If you know about mirror descent, then you can skip until the BLR section.


## Descending the simplex valley
Consider the ($$d-1$$)-dimensional probability simplex $$\mathcal{X}=\Delta^{d-1} = \{x:\|x\|_1 = 1,x\in[0,1]^d\}$$, where $$\|\cdot\|_1$$ is the L1 norm (the sum of the entries in absolute values).
...
The short answer is no. Life is not always that simple(x).
...
We can use *projected gradient descent* and simply project $$x_{t+1}$$ on $$\mathcal{X}$$, where the projection is often an easy operation.
A projection on the simplex exists [(Condat, 2014)](https://optimization-online.org/2014/08/4498/), but this one is not exactly easy.
Another way is to use a proper distance since Euclidean distances do not make sense for distributions.
If you want to measure the difference between two probability distributions, look no further than the [Kullback-Leibler divergence](https://en.wikipedia.org/wiki/Kullback–Leibler_divergence), which is the de facto distance between distributions.

Is there a specific rule for choosing the distance (i.e., the second order term in the gradient descent sub-problem)?
Kind of. More details next.

## Bregman divergence
Let's replace the second-order term in $$\eqref{eq:gradient-descent}$$ with a more general divergence term
...
where $$B_{\psi}(x,x_t)$$ is the [Bregman divergence](https://en.wikipedia.org/wiki/Bregman_divergence) associated with $$\psi$$
...
The function $$\psi$$ is strictly convex and differentiable, and should ideally capture the non-linear (second and higher-order) divergence terms, e.g., $$\psi(x) = \frac{1}{2}\|x\|^2$$ recovers the Euclidean metric as in vanilla gradient descent above.
...
We can now impose our metric assumptions directly on $$\psi$$.
...
As for the probability simplex case $$\Delta^d$$, we can generate the appropriate divergence using the negative entropy as the potential function $$\psi(x) = \sum_{i=1}^d x_i \log x_i$$ so that $$B_{\psi}(x,x') = \sum_{i=1}^d x_i \log \frac{x_i}{x_i'}$$, which is the KL-divergence.
Note that the additive terms cancel out because $$x, x' \in \Delta^d$$, where the general (unnormalized) case yields instead a divergence known as the *Information divergence*.

If you do the math and solve $$\eqref{eq:mirror-descent}$$, you should get an exponentiated gradient descent step [(Kivinen and Warmuth, 1997)](https://www.sciencedirect.com/science/article/pii/S0890540196926127)
...
In other words, we take a *Euclidean* gradient descent step in log-space, and then take the *Softmax* of the output, which is the standard normalization operator and is quite often used in deep learning to generate a probability distribution vector from a vector of real numbers (e.g., a distribution over classes for prediction).
...
And I find this to be interesting.
Here is a (non-exhaustive) table from [Nock & Nielsen (2005)](https://franknielsen.github.io/SlidesVideo/BCBregmanBalls-ECML-2005.pdf) that shows the bregman divergences associated with different strictly convex functions and their functional averages (explained below) on the last column:

![Bregman divergences and their associated functional averages](/assets/img/bregman_nock_nielsen_2005.png)
*Figure 2: Bregman divergences and their associated functional averages. Some other divergences not included in the above table are the Hellinger distance and the linear-exponential (linex) loss, e.g., see [Ackermann and Blömer (2009)](https://epubs.siam.org/doi/10.1137/1.9781611973068.118).*
...
There is also an interesting parallel in Bayes decision theory. When the risk of a classifier $$\hat{y}$$ is $$R(\hat{y}) = \frac{1}{2} \E_{x,y} \|\hat{y}(x) - y\|^2$$, then the Bayes optimal classifier is $$\hat{y}_\text{Bayes}(x) = \E_{y|x}[y]$$, which is like the arithmetic average.
...
Let's take the supremem for each $$\psi^\ast(g)$$
...
# Descending a valley of information
...
However, we will see that we can also work with distributions over points instead, which is a powerful framework that leads to inuitive understanding and derivation of optimization algorithms.

## Reformulating the objective
The regular optimization objective we usually consider looks like this
...
The authors used a linear approximation in $$\varepsilon$$, which results in a closed form max, which is $$f\left(x+r\frac{\nabla f(x)}{\|\nabla f(x)\|_2}\right)$$ for $$p=2$$, for example.
The gradient is then taken at this perturbed point *with respect to the unperturbed point*, i.e., $$z_t = \frac{\nabla f(x)}{\|\nabla f(x)\|_2} \big{|}_{x = x_t}$$ is fixed and taken as an adversarial noise and the step is taken using $$\nabla_x f(x + r z_t)$$.
...
It also comes from the fact that it can be interpreted as finding the [maximum a posteriori](https://en.wikipedia.org/wiki/Maximum_a_posteriori_estimation) estimator $$\theta$$ that has a prior $$\mathcal{N}(0, \frac{1}{\lambda} I_d)$$ and a posterior $$\mathcal{N}(x, \sigma^2 I_d)$$.

In fact, it has been shown recently that $$\eqref{eq:bayes}$$ is very much related to $$\eqref{eq:sam}$$.
The answer is in the title of this paper: ["SAM as an Optimal Relaxation of Bayes" (Möllenhoff and Khan, 2023)](https://arxiv.org/abs/2210.01620).
...
where $$p$$ is the prior of $$x$$, $$q$$ is the approximation of the posterior $$p(x \mid \xi)$$ with data $$\xi$$, and $$D_{\text{KL}}$$ is the KL-divergence, sort of like $$\Gamma(x)$$. Assuming a faithful *negative log-likelihood (NLL) loss* $$f(x) \propto -\log p(\xi \mid x)$$, the minimizer of $$\eqref{eq:oipr}$$ recovers the true posterior $$p(x \mid \xi)$$, which corresponds to [Bayesian inference](https://en.wikipedia.org/wiki/Variational_Bayesian_methods) as a special case (see below).

**Regarding notation**, perhaps practitioners of Bayesian methods would be slightly confused, so let's' clarify things a bit.
First, we are sticking with the optimization notation where $$f$$ is the loss, $$x$$ is the **parameter of the model, not distribution** (acting as the "latent" variable), and $$\xi$$ is the **dataset** (usually written in the literature as pairs of $$x_i$$ and $$y_i$$). Another source of confusion might be $$D_{\text{KL}}(q(x)\, \|\, p(x) )$$ as $$q(x)$$ should actually approach $$p(x \mid \xi)$$ as it gets better, not $$p(x)$$.
The reason for that is a bit subtle and would be clear in the following derivation.

Let's take a look at the [evidence lower bound (ELBO)](https://en.wikipedia.org/wiki/Evidence_lower_bound), which looks something like this
...
Maximizing the ELBO implies a better lower bound for the evidence. Cool, so how is this a special case of $$\eqref{eq:oipr}$$? It is straightforward, actually. Just plug in the *negative log-likelihood* loss
...
We now come to a question that is the crux of this blog post.
**Can we solve $$\eqref{eq:oipr}$$ with gradient descent**? What does it mean to take a gradient descent step on $$q$$? Does $$q_{t+1} = q_t - \eta_t \nabla_q \left(\E_{x \sim q} [f(x)] + D_{\text{KL}}(q(x)\, \|\, p(x) ) \right)$$ make sense?
...
Thus, a common way to describe and control probability distributions is by *parameterizing*[^dist_param_note] them under some constraints and assumptions, if any.
This way instead of working with functions, we work with their parameters instead.
In general, the constraints and assumptions describe the distribution family itself, and the parameter describe a member of the family.

Quite often, this parameter space is not Euclidean, i.e., the parameter entries do not vary linearly with respect to each other.
For example, the *natural* parameters that can fully characterize a member of the posterior family, such as the mean and covariance for normal distribution, controls the sampled outputs in a completely different manner.
The mean controls the position, while the variance controls the deviation or scattering.
It is obvious that position and deviation do not have a linear relationship.

We will see next that solving $$\eqref{eq:oipr}$$ with [**exponential family**](https://en.wikipedia.org/wiki/Exponential_family)[^ef_note] posteriors corresponds to *preconditioned gradient descent with an exponentially smoothed Hessian as a preconditioner*.
...
An interesting thing happens, for example, when you minimize the second-order taylor expansion of the functional $$F(q) := \E_{x \sim q} [f(x)]$$ around the mean $$\mu := \E_{x \sim q} [x]$$ (we will ignore the KL divergence term for now)
...
Observe that the first-order expansion simply recovers the regular objective.
Let $$\Sigma := \E_{x \sim q} [(x-\mu)(x-\mu)^T]$$ be the covariance of $$x$$, which is symmetric. Then,
...
Thus, approximating the objective $$\argmin_q F(q)$$ given the mean and covariance of $$q$$ gives
...
Interestingly, if we fix $$\Sigma = I$$, the Bayesian objective *implicitly flattens the minimizer* via a Hessian sharpness penalty, i.e., $$\langle \Sigma , \nabla^2 f(\mu) \rangle_\text{F} = \text{Tr}(\nabla^2 f(\mu))$$. In some sense, $$\Sigma$$ can be seen as a preconditioner (more details on this later). Therefore, optimizing the second-order approximation of $$\eqref{eq:oipr}$$'s objective by *tuning the preconditioner* $$\Sigma$$ might yield **lower objective values** than a fixed preconditioner (as in $$\Sigma = I$$), but this can happen at the expense of a **sharper Euclidean minimum** when $$f$$ is non-convex.
...
The variable $$\theta$$ is called the natural parameter (e.g., $$\theta = [\mu; \sigma^2]$$ for normal distributions), and $$A(\theta)$$ is the log-partition function $$\log \int h(x) e ^{ \langle \theta, T(x) \rangle} dx$$, which normalizes the probability $$q_\theta (x)$$.
...
The most straightforward description is the following: the FIM measures the expected ([observed](https://en.wikipedia.org/wiki/Observed_information)) information that samples $$x \sim q_{\theta}$$ have about $$\theta$$.
...
The latter identity can be shown by solving it directly (this time, using $$\int \nabla_\theta^2 = \nabla_\theta^2 \int$$).
...
This is interesting because that some kind of a Newton step is happening.
We will see the implications of this identity as we go on.
...
It follows mainly from $$\nabla_\theta \nu = \nabla^2 A(\theta) = \fisher(\theta)$$.
Focusing solely on the derivative operator, we can write
...
Natural gradient descent will yield the same iterates, so it is a proper gradient descent on $$q$$ that acts on the distribution rather than the natural parameter itself.
...
Assuming that the base measure $$h(x)$$ is constant, the natural gradient of the negative entropy $$-H(q_\theta)$$ is simply $$\theta$$, whereas the Euclidean gradient is $$\nabla^2 A(\theta) \theta$$ (see the appendix for details).
...
There is a simple interpretation for this phenomenon.
...
However, this is particularly because $$D$$ is the underlying metric in the regularizer (i.e., $$x^\text{opt}$$ is the minimizer s.t. $$\frac{1}{2}\|x\|_D^2 \leq 1$$).
For $$\eqref{eq:oipr}$$ with EF distributions $$q$$, the metric in the regularizer, or the entropy, is $$\fisher(\theta) = \nabla^2 A(\theta)$$.

Recall that the Euclidean gradient of the negative entropy $$-H(q_{\theta})$$ is a (invertible) linear transformation of $$\theta$$.
...
Natural gradient descent, similar to mirror descent, obtains the descent step from minimizing a sub-problem involving an approximation of the objective.
...
The differences appear mainly when the natural parameter space is constrained or when the second-order taylor expansion of the KL-divergence is not exact (for details, see [this](https://yorkerlin.github.io/posts/2021/12/Geomopt06)).

Now let's see how the natural gradient descent can be derived from mirror descent for EF distributions.
Recalling the duality $$\nu = \nabla A(\theta)$$ and $$\theta = \nabla A^*(\nu)$$, the KL-divergence can conveniently be rewritten as follows
...
Ok, so the most important question is... do we get something practical out of the BLR?
Well, I wouldn't have written this blog post if that wasn't the case.
...
The versatility of the BLR in deriving different practical algorithms is surprising given the simplicity and universality of its assumptions (EF distributions and natural gradients) and the theory-driven derivations, which leads to principled algorithmic practices that are mainly experiment and engineering-driven.

Next, we will show derivations for some practical algorithms, but before that, we will go through preliminaries about sufficient statistics and the delta method, which can be safely skipped if the reader already knows enough about them.

Henceforth, we will set $$\Gamma(\cdot)=0$$ for simplicity, as this choice is orthogonal to the derivation of the algorithms.
...
The only interesting and very useful part of practical algorithms that the BLR did not cover is momentum.
...
The straight through estimator is simple; we take a normal gradient descent step by pretending the weights are continuous but happen to be have discrete values, and then we project back to the discrete space after the step.
...
Letting $$s_t := \frac{1}{\tau} \left[ \frac{1-\tilde{x}_t^2}{1-\tanh(\theta_t)^2} \right]$$, we can write it as follows
...
Thus, we slightly generalize the first assumption of the BLR assumption to account for mixtures.
...
These look quite similar to the non-mixture case with an extra term from the latent variable, so how would the updates be different here?
The main difference is that the BLR has an entropy term, so now the BLR step would not exactly have the same exponential smoothing and the convenient cancellations in the updates.
Here, we have to consider the gradient (and Hessian) of the entropy of the whole mixture with $$\log q_\theta(x) = \log \sum_{i=1}^{K} \pi_k q_\theta(x|z=k)$$.
...
This is an interesting part that I won't talk much about, mainly to maintain focus on the optimization perspective of the BLR step, but also due to my lack of expertise in probabilistic inference.

The BLR can actually be used to derive inference algorithms, such as expectation-maximization and variational inference.
This can be done by setting the loss to be proportional to the negative log-likelihood, i.e.,  $$f(x) \propto -\log p(\xi|x)$$.
...
If you think that the BLR is another Bayesian [just-so story](https://en.wikipedia.org/wiki/Just-so_story) [(Bowers and Davis, 2012)](https://pubmed.ncbi.nlm.nih.gov/22545686/) but in the context of optimization, then you might be slightly mistaken.
In fact, state-of-the-art algorithms that outperform Adam on deep learning tasks can be derived from the BLR, e.g., the recently proposed Improved Variational Online Newton (IVON) [(Shen et al., 2024)](https://arxiv.org/abs/2402.17641).

I'm very interested in seeing what other optimization tools can be explained under the BLR framework, and whether a Bayesian lens can reveal anything we might have missed.
For example, the square root in the preconditioner in Adam and co. has a very different interpretation under Bayes (Lin et al., 2024, [Amari et al., 2024](https://arxiv.org/abs/2006.10732)), e.g., the algorithm proposed by Möllenhoff and Khan (2023) uses the absolute value instead of the square.

The main directions that I am interested in are the following:
...
- The learning rate is the most important (and sometimes only) hyperparameter of gradient descent algorithms. Tuning the learning rate adaptively is a very hot topic in optimization, often called [*parameter-free optimization (Orabona and Pál, 2016)*](https://github.com/bremen79/parameterfree). Another aspect that is worth a harder look and is not as well-understood is learning rate schedules, which is intimately related but not exactly the same. This includes, in particular, cyclic learning rates. On a side note, [Defazio et al. (2024)](https://github.com/facebookresearch/schedule_free) have developed an algorithm that seem to achieve the "pareto-optimal" schedule, but the theory is still not complete and the algorithm needs the right initial learning rate, which can have a very wide range. Coming back to cyclic learning rates, [Goujaud et al. (2022)](https://fa.bianp.net/blog/2022/cyclical/) show that a cycle of two learning rates on a quadratic problem with bimodal eigenvalue distribution can be (provably!) better than using the optimal (single) learning rate. I have a feeling that this could be explained and generalized under the BLR framework using multi-modal distributions, e.g., mixtures. It would be interesting to explore how to incorporate hyperparameter (hyperprior?) tuning in the BLR. This might also shed some light on other sophisticated techniques such as acceleration and Polyak step sizes.
